...
#ifdef CONFIG_MARS_EXPECTED_RESOURCES
int expected_resources = CONFIG_MARS_EXPECTED_RESOURCES;
#else
int expected_resources = 12;
#endif
...
int make_log_init(struct mars_dent *dent)
{
	...
	if (unlikely(!aio_dent)) {
		...
		status = -ENOENT;
		if (switch_on &&
		    rot->todo_primary && !rot->is_primary && !rot->old_is_primary) {
...
int make_primary(struct mars_dent *dent)
{
	...
	rot->todo_primary =
		 ...
		 * do not shortly interrupt any running fetch.
		 */
		(rot->todo_primary ||
		 !rot->fetch_brick ||
		 ...
		 !rot->start_jiffies ||
		 !rot->fetch_jiffies ||
		 jiffies > rot->start_jiffies + 30 * HZ ||
